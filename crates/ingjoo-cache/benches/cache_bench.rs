...
fn bench_cache_put_get(c: &mut Criterion) {
    ...
    c.bench_function("cache_put_get_scope_access", |b| {
        b.iter(|| {
            cache.put_scope_access(
                "scope1",
                "user1",
                BenchMember { role: "admin".to_string() },
            );
...
fn bench_cache_hit(c: &mut Criterion) {
    ...
    for i in 0..1000 {
        cache.put_scope_access(
            "scope",
            &format!("user_{}", i),
            BenchMember { role: format!("role_{}", i) },
        );
    }
            ...
            || {
                ...
                for i in 0..100 {
                    cache.put_scope_access(
                        "scope",
                        &format!("user_{}", i),
                        BenchMember { role: "reader".to_string() },
                    );
                }
                cache
            },
    ...
    c.bench_function("cache_put_get_user", |b| {
        b.iter(|| {
            cache.put_user(
                "u1",
                "u1@test.com",
                BenchUser { id: "u1".to_string(), email: "u1@test.com".to_string() },
            );
